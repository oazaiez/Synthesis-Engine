...
public class BvAnd extends Function {
    ...
    public Vector<Object> returnValue(Vector<Expr> v, Map<Expr, Vector<Object>> set, Context ctx)
    {
    	...
    	for(int i=0; i<vector.get(0).size(); i++)
    	{
    		...
    		BitVecExpr e = ctx.mkBVAND((BitVecExpr) vector.get(0).get(i), (BitVecExpr) vector.get(1).get(i));
    ...
    public Expr makeExpression(Vector<Expr> v, Context ctx)
    {
    	if(v.size()==2)
    	{
    	Expr e = ctx.mkBVAND((BitVecExpr) v.get(0), (BitVecExpr) v.get(1));
    	return e;
    	}
    	else return null;
    }
